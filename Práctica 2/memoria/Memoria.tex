...
Como hemos hecho para el anterior caso de estudio, resumiremos todas la información que hemos conseguido extraer. Toda esta información puede consultarse en sus respectivas secciones, más desarrolladas y con el contexto completo que nos ha hecho llegar a estas conclusiones.

En primer lugar, no hemos conseguido relacionar, usando gráficas simples, el uso del transporte público ni con la cantidad de ingresos mínimos para llegar a fin de mes ni con el número de miembros del hogar, como se muestra en \customcite{stcase02_relacion_tpublico_tprivado:figure} y en \customcite{stcase02_relacion_tpublico_resto_variables:figure}. Esto ha motivado el estudio posterior que hemos realizado apoyándonos en las \emph{clusterizaciones}.

En segundo lugar, hemos obtenido dos perfiles claros de hogares. En un segmento, aquellos con pocos miembros en el hogar, con poco gasto y con un perfil de vivienda muy marcado. En un segundo segmento, aquellos con más miembros, gasto y otro tipo de vivienda.

Esto es lógico. Un mayor número de miembros supone un mayor gasto. Además, hay viviendas que están más adecuadas a un número mayor de personas, y por esto el tipo de vivienda ha servido para separar bien los dos segmentos encontrados.

En tercer lugar, todos los algoritmos han obtenido resultados competentes. A diferencia del anterior caos de estudio, \emph{DBSCAN} no ha obtenido resultados desastrosos. Todos los algoritmos han obtenido una segmentación que es acorde con lo que hemos comentado previamente.
...
\begin{figure}[H]
    ...
    \includegraphics[width = 0.9 \textwidth]{stcase03_pairplot}
    \caption{Gráfico en el que se muestran todas las combinaciones de parejas de variables involucradas en el caso de estudio}
    \label{stcase01_pairplot:figura}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_resultadografico_kmeans}
        \caption{\lstinline{K-Means}}
    \end{subfigure}
    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_resultadografico_DBSCAN}
        \caption{\lstinline{DBSCAN}}
    \end{subfigure}

    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_resultadografico_BIRCH}
        \caption{\lstinline{BIRCH}}
    \end{subfigure}
    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_resultadografico_MEANSHIFT}
        \caption{\lstinline{Mean-Shift}}
    \end{subfigure}

    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_resultadografico_AGLOMERATIVO}
        \caption{\emph{Clustering} aglomerativo}
    \end{subfigure}
...
\begin{center}
    ...
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
        ...
        \hline
            ...
            kmeans &  4595.52 &  1.25 &  4768.00 &  1751.00 &  1.05 &  0.34 &  2 &  0.90 \\
            dbscan &  2132.88 &  2.44 &  2384.00 &  1235.69 &  0.53 &  0.17 &  4 &  0.91 \\
            birch &  4271.04 &  1.33 &  4768.00 &  1483.00 &  0.14 &  0.32 &  2 &  0.93 \\
            mean shift &  4595.52 &  1.25 &  4768.00 &  1751.00 &  86.54 &  0.34 &  2 &  0.90 \\
            agglomerative &  3987.63 &  1.32 &  4768.00 &  1931.00 &  3.03 &  0.32 &  2 &  0.88 \\
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_metricagrafica_silhouette}
        \caption{Silhouette}
    \end{subfigure}
    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_metricagrafica_calinski}
        \caption{Calinski}
    \end{subfigure}

    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_metricagrafica_davies}
        \caption{Davies}
    \end{subfigure}
    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_metricagrafica_time}
        \caption{Tiempo de ejecución}
    \end{subfigure}

    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_metricagrafica_total_clusters}
        \caption{Número total de clusters}
    \end{subfigure}
    \begin{subfigure}[b]{0.45 \textwidth}
        \includegraphics[width = 0.9 \textwidth]{stcase03_metricagrafica_total_unbalanced}
        \caption{Métrica \emph{unbalanced}}
    \end{subfigure}
...
\subsection{Interpretación de los resultados de \emph{clustering}} \label{stcase03_resultados:seccion}
...
En esta ocasión, hemos decidido ajustar los parámetros de los cinco algoritmos con los que estamos trabajando. De esta forma, la comparación entre los algoritmos que hemos hecho en \customcite{stcase03_resultados:seccion} es más justa, pues en todos hemos usado los mejores parámetros que hemos podido encontrar. Para hacer esto, los parámetros encontrados en esta sección son los que hemos aplicado a la hora de calcular las \emph{clusterizaciones} anteriormente.

Como ya hemos explorado este proceso para cuatro algoritmos, y como no aporta nada nuevo al estudio del problema a resolver, solo mostramos los resultados obtenidos para el algoritmo que nos falta, el de \emph{clustering aglomerativo}:

\begin{table}[H]
\begin{center}
    \begin{tabular}{|c|c|c|c|c|}
        \hline
            Número de clusters & Tipo Enlace & Silhouette & Calinski & unbalanced \\
        \hline

            \textbf{2} & \textbf{complete} &  \textbf{0.32} &  \textbf{3987.63} &  \textbf{0.88} \\
            2 & single &  0.27 &  3788.89 &  1.00 \\
            2 & ward &  0.31 &  3967.61 &  0.87 \\
            3 & complete &  0.26 &  2831.62 &  0.77 \\
            3 & single &  0.20 &  2753.31 &  0.91 \\
            3 & ward &  0.29 &  3692.48 &  0.96 \\
            4 & complete &  0.24 &  2240.70 &  0.72 \\
            4 & single &  0.17 &  2132.88 &  0.91 \\
            4 & ward &  0.29 &  3338.44 &  0.87 \\
            5 & complete &  0.19 &  2258.26 &  0.85 \\
            5 & single &  0.16 &  1601.10 &  0.79 \\
            5 & ward &  0.28 &  3136.34 &  0.90 \\

        \hline
    \end{tabular}
\end{center}
    \caption{Resultados del ajuste de los parámetros de \emph{Clustering Aglomerativo}}
    \label{resultados_stcase01:tabla}
\end{table}

Como siempre, nos quedamos con los parámetros que mejor valor de \emph{Silhouette} generan. Tenemos un balanceo muy bueno, a pesar de que no sea el mejor, y por tanto, sabemos que no estamos abusando de resultados triviales que generan buenos valores de \emph{Silhouette}.

\pagebreak

\subsection{Interpretación de la segmentación}

Como hemos hecho en los anteriores casos de estudio, resumiremos todas la información que hemos conseguido extraer. Toda esta información puede consultarse en sus respectivas secciones, más desarrolladas y con el contexto completo que nos ha hecho llegar a estas conclusiones.

En primer lugar, el haber usado únicamente variables discretas ha complicado enormemente el extraer información de utilidad a partir del análisis exploratorio de datos. Así que la información sobre el problema que hemos extraído se apoya completamente en la segmentación realizada. Esto nos muestra que esta técnica es una herramienta muy útil para la extracción de información en conjunto de datos \entrecomillado{no preparados de antemano}.

Gracias a dicha segmentación, hemos logrado obtener dos perfiles claros de hogar. Uno en el que tenemos muchas habitaciones por miembro, con un tipo de vivienda muy marcado. Otro segmento, con menos habitaciones por persona y con otro tipo de vivienda.

Es destacable lo bien que ha funcionado la variable \emph{tipo de vivienda} para separar la población en los dos segmentos que hemos comentado. Por otro lado, la variable \emph{capacidad para llegar a fin de mes} no ha servido para realizar una buena separación. Todo lo contrario. En \customcite{stcase03_resultadosgraficos:figure} podemos ver que la clusterización mezcla mucho los elementos respecto a esta variable. Por tanto, podría ser beneficioso eliminar esta variable de nuestro conjunto de datos, mejorando las métricas que hemos usado para evaluar los distintos algoritmos.

De hecho, esto es razonable. Al estar considerando hogares que disponen de una vivienda adicional, parece que la capacidad para llegar a fin de mes no es del todo relevante (aunque estamos seguros de que en muchas ocasiones, a pesar de disponer de viviendas adicionales, el hogar puede pasar apuros económicos, aunque esto no debería ser lo común).

Todos los algoritmos han obtenido resultados mediocres, respecto a los resultados obtenidos en los otros casos de estudio. Además, esto es más grave al tener en cuenta que hemos ajustado los valores de los parámetros para todos los algoritmos.

Esto puede deberse a que las variables no continuas no funcionan demasiado bien con estos algoritmos de clusterización, como sabemos de teoría. Sin embargo, no podemos descartar la hipótesis de que exclusivamente los datos de la vivienda sean suficientes para segmentar a la población de forma efectiva. A pesar de esto, parece más probable que sea un problema de la naturaleza discreta de las variables a que la información de la vivienda sea muy débil respecto a otro tipo de información, como por ejemplo información relativa a la conducta de las personas (si van a cenar fuera de casa, el gasto en transporte, \ldots)
