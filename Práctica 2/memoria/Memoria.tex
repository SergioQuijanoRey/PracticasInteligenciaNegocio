...
También es destacable el uso que hacemos de esta información. En todos los algoritmos en los que nos ha hecho fijar el número total de \emph{clusters}, hemos usado este valor. Por tanto, estamos usando esta exploración de parámetros y el algoritmo \emph{K-Means} como un método \emph{data-driven} para fijar los parámetros de otros algoritmos.
...
Claramente vemos que, con ninguno de los parámetros explorados que no provoca resultados triviales, no conseguimos realizar una asignación balanceada. Así que lo único que importa en la selección de estos parámetros es el punto que dejamos aislado. Cuanto más alejado esté este punto del resto de puntos, mejores métricas obtendremos.
...
\begin{enumerate}
    \item Realizar un borrado de \emph{outliers} más intenso, para evitar que \emph{DBSCAN} encuentre esos puntos aislados sobre los que colocar un \emph{cluster}. Sin embargo, a vista de la distribución de datos con la que trabajamos, esto no parece razonable
